...
static D3D_RESOURCE_RETURN_TYPE sm4_return_type(const struct hlsl_type *type)
{
        ...
        unsigned int *writemask, const struct hlsl_deref *deref, const struct hlsl_type *data_type)
{
    ...
    else if (var->is_input_semantic)
    {
        ...
        {
            struct hlsl_reg hlsl_reg = hlsl_reg_from_deref(deref);
    ...
    switch (expr->node.data_type->base_type)
    {
        ...
        case HLSL_TYPE_INT:
        {
            switch (expr->op)
            {
                ...
                case HLSL_OP2_RSHIFT:
                    write_sm4_binary_op(buffer, VKD3D_SM4_OP_ISHR, &expr->node, arg1, arg2);
                    break;
        ...
        case HLSL_TYPE_UINT:
        {
            switch (expr->op)
            {
                ...
                case HLSL_OP2_MOD:
                    write_sm4_binary_op_with_null(buffer, VKD3D_SM4_OP_UDIV, &expr->node, arg1, arg2, 1);
                    break;
                ...
                case HLSL_OP2_RSHIFT:
                    write_sm4_binary_op(buffer, VKD3D_SM4_OP_USHR, &expr->node, arg1, arg2);
                    break;
            ...
            switch (expr->op)
            {
                case HLSL_OP1_CAST:
                {
                    ...
                    switch (src_type->base_type)
                    {
                        case HLSL_TYPE_HALF:
                        case HLSL_TYPE_FLOAT:
                            hlsl_fixme(ctx, expr->node.loc, "Casts from float/half to bool are not implemented.\n");
                            break;

                        case HLSL_TYPE_INT:
                        case HLSL_TYPE_UINT:
                            write_sm4_conditional_op(buffer, &expr->node, arg1, 0xffffffff, 0);
                            break;
                        ...
                        case HLSL_TYPE_DOUBLE:
                            hlsl_fixme(ctx, expr->node.loc, "Casts from double to bool are not implemented.\n");
                            break;
                ...
                case HLSL_OP2_GREATER:
                {
                    ...
                    {
                        case HLSL_TYPE_FLOAT:
                            write_sm4_binary_op(buffer, VKD3D_SM4_OP_LT, &expr->node, arg2, arg1);
                            break;

                        case HLSL_TYPE_BOOL:
                        case HLSL_TYPE_INT:
                            write_sm4_binary_op(buffer, VKD3D_SM4_OP_ILT, &expr->node, arg2, arg1);
                            break;

                        case HLSL_TYPE_UINT:
                            write_sm4_binary_op(buffer, VKD3D_SM4_OP_ULT, &expr->node, arg2, arg1);
                            break;
                ...
                case HLSL_OP2_LEQUAL:
                {
                    ...
                    {
                        case HLSL_TYPE_FLOAT:
                            write_sm4_binary_op(buffer, VKD3D_SM4_OP_GE, &expr->node, arg2, arg1);
                            break;

                        case HLSL_TYPE_BOOL:
                        case HLSL_TYPE_INT:
                            write_sm4_binary_op(buffer, VKD3D_SM4_OP_IGE, &expr->node, arg2, arg1);
                            break;

                        case HLSL_TYPE_UINT:
                            write_sm4_binary_op(buffer, VKD3D_SM4_OP_UGE, &expr->node, arg2, arg1);
                            break;
