        ...
        struct hlsl_ir_node *value, const struct vkd3d_shader_location *loc)
{
    ...
    store->resource.var = resource;
    hlsl_src_from_node(&store->resource.offset, offset);
        ...
        struct hlsl_ir_node *val, const struct vkd3d_shader_location *loc)
{
    ...
    if (components == 1)
        type = hlsl_get_scalar_type(ctx, base_type);
    else
        type = hlsl_get_vector_type(ctx, base_type, components);
        ...
        struct clone_instr_map *map, struct hlsl_ir_resource_load *src)
{
    ...
    if (!(dst = hlsl_new_resource_load(ctx, src->node.data_type, src->load_type, src->resource.var,
            map_instr(map, src->resource.offset.node), src->sampler.var, map_instr(map, src->sampler.offset.node),
            map_instr(map, src->coords.node), map_instr(map, src->texel_offset.node), &src->node.loc)))
        return NULL;
        ...
        struct vkd3d_shader_code *out, struct vkd3d_shader_message_context *message_context)
{
    ...
    if ((func = hlsl_get_function(&ctx, entry_point)))
    {
        RB_FOR_EACH_ENTRY(decl, &func->overloads, struct hlsl_ir_function_decl, entry)
        {
            ...
            if (entry_func)
            {
                hlsl_fixme(&ctx, &decl->loc, "Overloaded entry point.");
                break;
            }
            entry_func = decl;
        }
