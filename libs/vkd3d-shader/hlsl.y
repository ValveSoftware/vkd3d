        ...
        struct hlsl_ir_node *index, const struct vkd3d_shader_location *loc)
{
            ...
            && expr_type->sampler_dim != HLSL_SAMPLER_DIM_GENERIC)
    {
        ...
        if (!(resource_load = hlsl_new_resource_load(ctx, expr_type->e.resource_format, HLSL_RESOURCE_LOAD,
                object_load->src.var, object_load->src.offset.node, NULL, NULL, &coords_load->node, NULL, loc)))
            return false;
        ...
        const struct parse_initializer *params, const struct vkd3d_shader_location *loc)
{
    ...
    if (type->type == HLSL_CLASS_MATRIX || (type->dimx != 1 && type->dimx != 4))
    {
        struct vkd3d_string_buffer *string;
        ...
        const struct vkd3d_shader_location *loc, const char *name, enum hlsl_sampler_dim dim)
{
    ...
    if (!(load = hlsl_new_resource_load(ctx, hlsl_get_vector_type(ctx, HLSL_TYPE_FLOAT, 4), HLSL_RESOURCE_SAMPLE,
            sampler_load->src.var, sampler_load->src.offset.node, NULL, NULL, coords, NULL, loc)))
        return false;
    list_add_tail(params->instrs, &load->node.entry);
    return true;
}
        ...
        const char *name, const struct parse_initializer *params, const struct vkd3d_shader_location *loc)
{
    ...
    else if (!strcmp(name, "SampleLevel"))
    {
        ...
        if (!(load = hlsl_new_sample_lod(ctx, object_type->e.resource_format,
                ...
                sampler_load->src.var, sampler_load->src.offset.node, coords, offset, lod, loc)))
            return false;
        list_add_tail(instrs, &load->node.entry);
        return true;
    }
      ...
      var_modifiers type var_identifier '(' parameters ')' colon_attribute
        {
            ...
            if (!($$.decl = hlsl_new_func_decl(ctx, type, $5, &$7.semantic, 0, NULL, @3)))
                YYABORT;
            $$.name = $3;
