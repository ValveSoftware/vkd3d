...
static struct hlsl_ir_expr *add_expr(struct hlsl_ctx *ctx, struct list *instrs, enum hlsl_ir_expr_op op,
        struct hlsl_ir_node *operands[HLSL_MAX_OPERANDS], struct hlsl_type *cast_types[HLSL_MAX_OPERANDS],
        struct hlsl_type *type, struct vkd3d_shader_location *loc)
{
    ...
    for (i = 0; i < HLSL_MAX_OPERANDS; ++i)
    {
        ...
        if (!operands[i])
            break;
        if (hlsl_types_are_equal(operands[i]->data_type, cast_types[i]))
            continue;
        if (operands[i]->data_type->dimx * operands[i]->data_type->dimy != 1
                && operands[i]->data_type->dimx * operands[i]->data_type->dimy != cast_types[i]->dimx * cast_types[i]->dimy)
        ...
        if (!(cast = hlsl_new_cast(ctx, operands[i], cast_types[i], &operands[i]->loc)))
            return NULL;
        ...
        enum hlsl_ir_expr_op op, struct hlsl_ir_node *arg, struct vkd3d_shader_location *loc)
{
    ...
    struct hlsl_type *cast_types[HLSL_MAX_OPERANDS] = {arg->data_type};

    return add_expr(ctx, instrs, op, args, cast_types, arg->data_type, loc);
}

static struct list *add_unary_arithmetic_expr_last(struct hlsl_ctx *ctx, struct list *instrs,
        enum hlsl_ir_expr_op op, struct vkd3d_shader_location *loc)
{
    add_unary_arithmetic_expr(ctx, instrs, op, node_from_list(instrs), loc);
...
static struct hlsl_ir_expr *add_ternary_expr(struct hlsl_ctx *ctx, struct list *instrs,
        enum hlsl_ir_expr_op op, struct hlsl_ir_node *arg1, struct hlsl_ir_node *arg2,
        struct hlsl_ir_node *arg3, struct vkd3d_shader_location *loc)
{
    struct hlsl_type *cast_types[HLSL_MAX_OPERANDS];
    enum hlsl_base_type base = expr_common_base_type(arg2->data_type->base_type, arg3->data_type->base_type);
    enum hlsl_type_class type;
    unsigned int dimx, dimy;
    struct hlsl_ir_node *args[HLSL_MAX_OPERANDS] = {arg1, arg2};

    /* FIXME: use the shape of the first parameter */
    if (!expr_common_shape(ctx, arg2->data_type, arg3->data_type, loc, &type, &dimx, &dimy))
        return NULL;

    cast_types[0] = get_numeric_type(ctx, type, HLSL_TYPE_BOOL, dimx, dimy);
    cast_types[1] = get_numeric_type(ctx, type, base, dimx, dimy);
    cast_types[2] = cast_types[1];

    return add_expr(ctx, instrs, op, args, cast_types, cast_types[1], loc);
}

static struct list *add_ternary_expr_last(struct hlsl_ctx *ctx, struct list *instrs1, struct list *instrs2,
        struct list *instrs3, enum hlsl_ir_expr_op op, struct vkd3d_shader_location *loc)
{
    struct hlsl_ir_node *arg1 = node_from_list(instrs1), *arg2 = node_from_list(instrs2), *arg3 = node_from_list(instrs3);

    list_move_tail(instrs1, instrs2);
    list_move_tail(instrs1, instrs3);
    vkd3d_free(instrs2);
    vkd3d_free(instrs3);
    add_ternary_expr(ctx, instrs1, op, arg1, arg2, arg3, loc);

    return instrs1;
}

static struct list *add_binary_arithmetic_expr_last(struct hlsl_ctx *ctx, struct list *instrs1, struct list *instrs2,
        enum hlsl_ir_expr_op op, struct vkd3d_shader_location *loc)
{
    ...
    list_move_tail(instrs1, instrs2);
    vkd3d_free(instrs2);
    add_binary_arithmetic_expr(ctx, instrs1, op, arg1, arg2, loc);
...
static struct list *add_binary_logical_expr_last(struct hlsl_ctx *ctx, struct list *instrs1, struct list *instrs2,
        enum hlsl_ir_expr_op op, struct vkd3d_shader_location *loc)
{
    struct hlsl_ir_node *arg1 = node_from_list(instrs1), *arg2 = node_from_list(instrs2);
    ...
    list_move_tail(instrs1, instrs2);
    vkd3d_free(instrs2);
    add_binary_logical_expr(ctx, instrs1, op, arg1, arg2, loc);
...
{
    struct hlsl_ir_expr *x;
    struct hlsl_ir_constant *dimx;

    assert(type->type == HLSL_CLASS_MATRIX);

    if (!(dimx = hlsl_new_uint_constant(ctx, type->dimx, *loc)))
        return NULL;
    list_add_tail(instrs, &dimx->node.entry);

    if (!(x = add_binary_arithmetic_expr(ctx, instrs, HLSL_OP2_MOD, idx, &dimx->node, loc)))
        return NULL;
        ...
        struct hlsl_ir_node *idx, struct vkd3d_shader_location *loc)
{
    ...
    if (!(y = add_binary_arithmetic_expr(ctx, instrs, HLSL_OP2_DIV, idx, &dimx->node, loc)))
        return NULL;
        ...
        struct hlsl_ir_node *idx, struct vkd3d_shader_location *loc)
{
    ...
    if (!(x = component_x_from_idx(ctx, instrs, type, idx, loc)))
        return NULL;

    if (!(y = component_y_from_idx(ctx, instrs, type, idx, loc)))
        return NULL;

    return component_offset(ctx, instrs, type, x, y, loc);
}
...
register_opt:
      ':' KW_REGISTER '(' any_identifier ')'
        {
            if (!parse_reservation_index($4, &$$.type, &$$.index))
    ...
    | ':' KW_REGISTER '(' any_identifier ',' any_identifier ')'
        {
            if (parse_reservation_index($6, &$$.type, &$$.index))
            {
                FIXME("Ignoring reservation shader target %s.\n", debugstr_a($4));
                $$.space = 0;
            }
            else if (parse_reservation_space($6, &$$.space))
            {
                if (!parse_reservation_index($4, &$$.type, &$$.index))
                    hlsl_error(ctx, @4, VKD3D_SHADER_ERROR_HLSL_INVALID_RESERVATION,
                            "Invalid register reservation '%s'.", $4);
    ...
    | ':' KW_REGISTER '(' any_identifier ',' any_identifier ',' any_identifier ')'
        {
            ...
            if (!parse_reservation_space($8, &$$.space))
...
unary_expr:
    ...
    | '-' unary_expr
        {
            $$ = add_unary_arithmetic_expr_last(ctx, $2, HLSL_OP1_NEG, &@1);
        }
    | '~' unary_expr
        {
...
mul_expr:
    ...
    | mul_expr '*' unary_expr
        {
            $$ = add_binary_arithmetic_expr_last(ctx, $1, $3, HLSL_OP2_MUL, &@2);
        }
    | mul_expr '/' unary_expr
        {
            $$ = add_binary_arithmetic_expr_last(ctx, $1, $3, HLSL_OP2_DIV, &@2);
        }
    | mul_expr '%' unary_expr
        {
            $$ = add_binary_arithmetic_expr_last(ctx, $1, $3, HLSL_OP2_MOD, &@2);
        }
...
add_expr:
    ...
    | add_expr '+' mul_expr
        {
            $$ = add_binary_arithmetic_expr_last(ctx, $1, $3, HLSL_OP2_ADD, &@2);
        }
    | add_expr '-' mul_expr
        {
...
shift_expr:
    ...
    | shift_expr OP_RIGHTSHIFT add_expr
        {
            $$ = add_binary_shift_expr_last(ctx, $1, $3, HLSL_OP2_RSHIFT, &@2);
        }
...
bitxor_expr:
    ...
    | bitxor_expr '^' bitand_expr
        {
            $$ = add_binary_bitwise_expr_last(ctx, $1, $3, HLSL_OP2_BIT_XOR, &@2);
        }
...
logicand_expr:
    ...
    | logicand_expr OP_AND bitor_expr
        {
            $$ = add_binary_logical_expr_last(ctx, $1, $3, HLSL_OP2_LOGIC_AND, &@2);
        }
...
conditional_expr:
    ...
    | logicor_expr '?' expr ':' assignment_expr
        {
            $$ = add_ternary_expr_last(ctx, $1, $3, $5, HLSL_OP3_MOVC, &@2);
        }
