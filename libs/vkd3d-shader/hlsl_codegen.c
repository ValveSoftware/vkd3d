        ...
        struct hlsl_ir_node *condition, struct hlsl_ir_node *if_true, struct hlsl_ir_node *if_false)
{
    ...
    if (!(name = hlsl_get_string_buffer(ctx)))
        return NULL;
    vkd3d_string_buffer_printf(name, "<conditional-%u>", counter++);
    var = hlsl_new_synthetic_var(ctx, name->buffer, if_true->data_type, condition->loc);
    hlsl_release_string_buffer(ctx, name);
    if (!var)
        return NULL;
    ...
    if (!(load = hlsl_new_load(ctx, var, NULL, if_true->data_type, condition->loc)))
        return NULL;
