...
struct d3d12_device
{
    ID3D12Device4 ID3D12Device4_iface;
    LONG refcount;
...
{
    return ID3D12Device4_QueryInterface(&device->ID3D12Device4_iface, iid, object);
}

static inline ULONG d3d12_device_add_ref(struct d3d12_device *device)
{
    return ID3D12Device4_AddRef(&device->ID3D12Device4_iface);
}

static inline ULONG d3d12_device_release(struct d3d12_device *device)
{
    return ID3D12Device4_Release(&device->ID3D12Device4_iface);
}

static inline unsigned int d3d12_device_get_descriptor_handle_increment_size(struct d3d12_device *device,
        D3D12_DESCRIPTOR_HEAP_TYPE descriptor_type)
{
    return ID3D12Device4_GetDescriptorHandleIncrementSize(&device->ID3D12Device4_iface, descriptor_type);
}
